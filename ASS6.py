...
plt.plot(t_d, integral_values, '.')
plt.xlabel(' $T_d$')
plt.ylabel(' Integral of measured intensity (A)')
...
#    exp_fit.append(a[0]*np.exp(1.1*a[1]*n)+0.07)
    ...
    exp_fit.append(0.52*a[0]*np.exp(0.7*a[1]*n)+0.07)
    
##    exp_fit.append(a[0]*np.exp(a[1]*n))
##    exp_fit_2.append(0.55*a[0]*np.exp(0.3*a[1]*n))
#plt.plot(d_mm, exp_fit, label = 'fit: '+str(round(1.1*a[0],2))+'*exp('+str(round(a[1],2))+'*d)+0.07')
